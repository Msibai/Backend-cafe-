...
router.post("/", (request, response) => {
    ...
    if (request.session.passwordAttempts > 9) {
      ...
      response.json({
        error:
          "Too many attempts. You must verify your account using two-factor authentication",
      });
      return;
    }
    ...
    let user = request.db
      ...
      .all([request.body.email, encryptedPassword]);
    user = user[0];
    ...
    if (
      user &&
      user.email &&
      request.session?.verification?.status === 0 &&
      request.session?.verification?.phone_number === user?.phone
    ) {
      ...
      if (
        !request.session.verification ||
        request.session?.verification?.status === -1
      ) {
        response.json({
          error: "You must verify your account using two-factor authentication",
        });
      } else {
        response.json({
